...
pub fn pop_filenames_from_query(query: &str) -> (String, Vec<String>) {
    ...
    for file in &files {
        ...
        let table_name = table_name.trim_end_matches(".csv");

        query = query.replace(file, &(&table_name).to_string());
    }
    let result = query;
    (result, files)
}
...
pub mod tests {
    ...
    fn should_pop_filenames_from_query() {
        ...
        assert_eq!(
            query,
            "SELECT * from oscar_age JOIN 'foo bar' WHERE Year=2014"
        );
        assert_eq!(
            files,
            vec!["test/fixtures/oscar_age.csv", "./test/foo bar.csv"]
        );
    }
}
